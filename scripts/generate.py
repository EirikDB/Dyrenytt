...
Skriv en naturlig, vennlig dialog på norsk bokmål. Sikt på 8–10 minutter, omtrent 1900–2400 ord – ikke lever et kort sammendrag. Dekk ALLE de oppgitte dyre-/kjæledyrsakene som er relevante (ikke hopp over gode saker), og prioriter de ferskeste. Hold hver sak kort – omtrent 2–4 setninger: hva som har skjedd og hvorfor det er relevant. Ta med SÅ MANGE saker som mulig som har substans – bredde er viktigere enn dybde, så heller mange korte saker enn få lange. Nevn gjerne hvor saken kommer fra (kilden er oppgitt for hver sak), for eksempel «ifølge NRK» eller «melder Nettavisen».
...
def main():
    ...
    seen = load_seen()
    animal = filter_unseen(animal, seen, now_ts)[:18]     # flere saker (bredde > dybde)
